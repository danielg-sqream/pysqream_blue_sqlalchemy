...
class TestDdl(TestBase):
    ...
    def test_get_tables(self):
        ...
        try:
            ...
            engine2.execute("create or replace table x (x int, y text, z datetime)")
            engine2.execute("create or replace foreign table  x_foreign (x int, y text, z datetime) wrapper parquet_fdw options (location = 'gs://qa-tests-b1/nemanja/test1.parquet', continue_on_error = 'False');")
            res = engine2.dialect.has_table(engine2.connect(), "x")
            assert (True, res)
            res = engine2.dialect.has_table(engine2.connect(), "x_foreign")
            assert (True, res)
        finally:
    ...
    def test_get_view(self):
        try:
            ...
            engine2 = create_engine(manual_conn_str, connect_args=connect_args)
            engine2.execute("create or replace table x (x int, y text, z datetime)")
            engine2.execute("create view view_z1 as select * from x")
            views = engine2.dialect.get_view_names(engine2.connect())
            assert "view_z1" in views
        finally:
            if engine2 is not None:
                engine2.dispose()
